    ...
    var methods = {
        ...
        show: function(overNode, iCateNo) {
            ...
            $(methods.aSubCategory[iCateNo]).each(function() {
                aHtml.push('<li class="gnb_ul_li"><a href="'+this.link_product_list+'">'+this.name+'</a></li>');
            });
            aHtml.push('</ul>');


            var offset = $(overNode).offset();
            $('<div class="gnb_ul position-absolute eng_hdr_sub_lt_13"></div>')
                .appendTo(overNode)
                .html(aHtml.join(''))
                .find('li').on({
